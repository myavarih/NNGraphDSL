...
\begin{enumerate}
    ...
    رشته Python
    \item \textbf{خروجی}: نوشتن به فایل \code{.py} یا چاپ در
    \code{stdout}
\end{enumerate}
\end{dfnbox}
...
\code{main.py}                & رابط CLI و \code{compile\_nng()} (۷۳ خط) \\
\bottomrule
\end{tabular}
\end{center}
...
\begin{codebox}{قالب خطا و هشدار}{code-error-format}
\begin{amzcode}{text}
...
هشدارها فقط در \code{stderr} چاپ می‌شوند و کامپایل را متوقف
نمی‌کنند.
...
\LR{Split}          & بدون ماژول   & \code{parts = torch.chunk(x,n,dim=d)} \\
...
\LR{NNGraph DSL} یک زنجیره کامپایلر چهار مرحله‌ای از \code{.nng} به
...
\midrule
استنتاج شکل        & انتشار شکل تنسور و تشخیص عدم تطابق در کامپایل \\
صادرکردن ONNX       & تولید گراف سازگار با ONNX به جای Python \\
داربست آموزش        & تولید حلقه آموزش با بهینه‌ساز و تابع خطا از \code{config} \\
